...
export class InsertOrUpdateTruck {
  ...
  async save() {
    // update truck_status
    this.item.truck_status = this.setCommingSoonStatus(this.truck_status);
    console.log('save() this.item.truck_status', this.item.truck_status);

    var max=0;
    if (!this.item.id) { //tạo mới
      let ds = await this.truckSrv.GetAllTrucks();
    ...
    let result = await this.truckSrv.findExist(this.item.postId);
    console.log('save() result', result);
    if (result.length > 0) {
      this.item.id = result[0].id;
      await this.truckSrv.Patch(this.item);
    } else {
      await this.truckSrv.Post(this.item);
    }
