...
@inject(ContactServiceImpl, DialogController, ValidationControllerFactory, DialogService)

export class InsertOrUpdateContact {
  ...
  async runUpdate(item) {
    await this.contactSrv.Patch(item).then(res => this.dialogcontroller.ok());
  }
  ...
  async runCreate(item){
    ...
    await this.contactSrv.Post(item).then(res => this.dialogcontroller.ok());
  }
