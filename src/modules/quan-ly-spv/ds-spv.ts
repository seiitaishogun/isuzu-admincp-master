...
export class ManHinhSPV implements ViewModelBase {
  items: any[];
  itemsCount: number;
  selectedItem: any;
  selectedList: any[];
  ...
  async runFilter() {
    ...
    //   delete this.filter.where["khachHangId"];
      ...
      await (this.asyncTask = Promise.all([
        this.spvSrv.GetAll(this.filter).then(rec => { this.items = rec }),
        // this.spvSrv.GetCount(this.filter).then(rec => { this.itemsCount = rec })
