...
@inject(TestdriveServiceImpl, DialogController, ValidationControllerFactory, DialogService)

export class InsertOrUpdateTestdrive {
  ...
  async runUpdate(item) {
    await this.testdriveSrv.Patch(item).then(res => this.dialogcontroller.ok());
  }
  ...
  async runCreate(item){
    ...
    await this.testdriveSrv.Post(item).then(res => this.dialogcontroller.ok());
  }
