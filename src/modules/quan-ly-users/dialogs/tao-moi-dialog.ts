...
@inject(UserServices, DialogController, ValidationControllerFactory, DialogService)
export class UserDialog {
  ...
  async runUpdate(item) {
    await this.userSrv.Patch(item).then(res => this.dialogcontroller.ok());
  }
  ...
  async runCreate(item) {
    item.isActive = true;
    await this.userSrv.Post(item).then(res => this.dialogcontroller.ok());
  }
