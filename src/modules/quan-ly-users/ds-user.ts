...
import { UserDialog } from './dialogs/tao-moi-dialog';
...
@inject(UserServices, DialogService, AuthenService)
export class ManHinhAppUser implements ViewModelBase {
  ...
  constructor(
    private AppUserSrv: UserServices,
  ...
  async runUpdate(item) {
    ...
    this.dialogService.open({ viewModel: UserDialog, model: this.selectedItem }).whenClosed((result) => {
