...
export class InsertOrUpdateLCV {
  ...
  async save() {
    ...
    if (!this.item.id) { //tạo mới
      let ds = await this.lcvSrv.GetAllLCVs();
    ...
    let result = await this.lcvSrv.findExist(this.item.postId);
    console.log('save() result', result);
    if (result.length > 0) {
      this.item.id = result[0].id;
      await this.lcvSrv.Patch(this.item);
    } else {
      await this.lcvSrv.Post(this.item);
    }
