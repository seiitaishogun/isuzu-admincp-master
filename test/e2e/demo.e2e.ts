...
describe('aurelia skeleton app', function() {
  ...
  it('should load the page and display the initial page title', async () => {
    await expect(poSkeleton.getCurrentPageTitle()).toBe('Welcome | ISUZU VIETNAM CO., LTD.');
  });

  it('should display greeting', async () => {
    await expect(poWelcome.getGreeting()).toBe('Chào mừng đến với ISUZU VIETNAM CO., LTD.');
  });
  ...
  it('should navigate to users page', async () => {
    ...
    await expect(poSkeleton.getCurrentPageTitle()).toBe('Github Users | ISUZU VIETNAM CO., LTD.');
  });
});
